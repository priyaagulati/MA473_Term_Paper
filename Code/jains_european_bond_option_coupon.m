function prc = jains_european_bond_option_coupon(m, n, gamma, T, K, kappa, tht, sig, r_0, r_min, r_max, T_0, z)

    % European bond option with coupons
    ...
    for i = 2 : n + 1    
        ...
        for j = 1 : z + 1
            P(j, i) = bond_price(kappa, tht, sig, r_min + (j - 1) * h, t, T) + (i / (2 * T)) * 0.1;
            P(m + 2 - j, i) = bond_price(kappa, tht, sig, r_max - (j - 1) * h, t, T) + (i / (2 * T)) * 0.1;
        end
    end    
    ...
    for i = 2 : n + 1    
        ...
        for j = 1 : z + 1
           V(j, i) = cr_V(j, i) + (i / (2 * T)) * 0.1;
           V(m + 2 - j, i) = cr_V(m + 2 - j, i) + (i / (2 * T)) * 0.1;
        end
        if mod(i, n / (2 * T)) == 0
            V(:, i) = V(:, i) + 0.1;
            cr_V(:, i) = cr_V(:, i) + 0.1;
