function prc = jains_bermudan_bond_option_coupon(m, n, gamma, T, kappa, tht, sig, r_0, r_min, r_max, z)

	% Bermudan option with coupons
    ...
    for i = 2 : n + 1    
        ...
        t = T - (i - 1) * k;
        for j = 1 : z + 1
            P(j, i) = bond_price(kappa, tht, sig, r_min + (j - 1) * h, t, T) + (i / T) * 0.04;
            P(m + 2 - j, i) = bond_price(kappa, tht, sig, r_max - (j - 1) * h, t, T) + (i / T) * 0.04;
        end
    end
